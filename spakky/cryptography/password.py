...
class Password:
    ...
    @overload
    def __init__(self, *, password: str, salt: Key, hash_type: HashType, iteration: int) -> None:
        ...
    ...
    def __init__(
        ...
        if hash is not None:
            components: list[str] = hash.split(":")
            components.pop(0)  # remove tag
            self.__hash_type = HashType(components[0].upper())
            self.__iteration = int(components[1])
            self.__salt = Key(
                base64=Base64Encoder.from_urlsafe(components[2]),
                url_safe=True,
            )
    ...
    def decompose(cls, password_hash: str) -> tuple[Key, HashType, int, str]:
        password: Password = Password(hash=password_hash)
        return (
            password.salt,
            password.hash_type,
            password.iteration,
    ...
    def challenge(self, password: str) -> bool:
        ...
        return self.hash == new_password.hash
